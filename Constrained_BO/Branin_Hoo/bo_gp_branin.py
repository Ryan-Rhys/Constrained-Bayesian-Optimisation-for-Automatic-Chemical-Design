"""
This module provides a constrained Bayesian Optimisation implementation using
a sparse GP for the regression model and a Bayesian Neural network (BNN) for the model of the constraint.
The acquisition function used is greedy expected improvement.
"""
...
def main(input_directory, output_directory):
    ...
    # We set the number of data collection iterations
    ...
    for iteration in range(num_iters):
        ...
        # We pick the next input

        # assuming bb_alpha_samples is different to n_samples

        bb_alpha_samples = 50

        next_inputs = sgp.batched_greedy_ei(bb_alpha, 1, np.min(X_tr_bran, 0), np.max(X_tr_bran, 0), bb_alpha_samples)
        ...
        for i in range(next_inputs.shape[0]):
            ...
            reg_scores.append([branin(tuple(next_inputs[i]))])

            if (next_inputs[i][0] - 2.5)**2 + (next_inputs[i][1] - 7.5)**2 <= 50:
        ...
        save_object(next_inputs, output_directory + "/next_inputs{}.dat".format(iteration))
        ...
        X_tr_bran = np.concatenate([X_tr_bran, next_inputs], 0)
...
if __name__ == '__main__':
    ...
    parser.add_argument("output_directory", nargs='?', help='the output directory to which plots are saved', default='13_Dec_2018')

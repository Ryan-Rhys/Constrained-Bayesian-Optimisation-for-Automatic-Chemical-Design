...
class SparseGP:
    ...
    def __init__(self, input_means, input_vars, training_targets, n_inducing_points):
        ...
        self.input_means_numpy = input_means # added 28th July
    ...
    def get_incumbent(self, bb_alpha, grid, bb_alpha_samples):
        ...
        function_grid = theano.function([X], m, givens={self.input_means: X, self.input_vars: 0 * X}, allow_input_downcast=True)
        function_grid_prob = theano.function([X], T.reshape(pred_probs[:, :, 1], [T.shape(X)[0], 1]), givens={self.input_means: X, self.input_vars: 0 * X}, allow_input_downcast=True)
        ...
        if np.max(p_on_grid) < 0.95:
            ...
            max_feasible_index = np.where(m_on_grid == np.max(m_on_grid.take(feasible_point_indices)))[0][0]
    ...
    def optimize_ei(self, bb_alpha, grid, lower, upper, incumbent, bb_alpha_samples):
        ...
        function_grid = theano.function([X], -log_ei - T.reshape(pred_log_probs[:, :, 1], [T.shape(X)[0], 1]), allow_input_downcast=True)
        function_scalar = theano.function([X], -log_ei[0, 0] - T.reshape(pred_log_probs[:, :, 1], [T.shape(X)[0], 1])[0, 0], allow_input_downcast=True)
        function_scalar_gradient = theano.function([X], -T.grad(log_ei[0, 0] + T.reshape(pred_log_probs[:, :, 1], [T.shape(X)[0], 1])[0, 0], X), allow_input_downcast=True)
    ...
    def batched_greedy_ei(self, bb_alpha, q, lower, upper, bb_alpha_samples, n_samples=1):
        ...
        pred_log_probs = LogSumExp(bb_alpha.network.output(x), 0) + T.log(1.0 / bb_alpha_samples)  # 2-D array of size (n_samples, 2) where the column 1 gives the log probability of the constraint being unsatisfied and column two gives the log probabilty of the constraint being satisfied

        function_grid = theano.function([x], -log_ei - T.reshape(pred_log_probs[:, :, 1], [T.shape(x)[0], 1])[:, 0], allow_input_downcast=True) # indices for pred_log_probs give the log probability of the constraint being satisfied
        function_scalar = theano.function([x], -log_ei[0] - T.reshape(pred_log_probs[:, :, 1], [T.shape(x)[0], 1])[0, 0], allow_input_downcast=True)
        function_scalar_gradient = theano.function([x], -T.grad(log_ei[0] + T.reshape(pred_log_probs[:, :, 1], [T.shape(x)[0], 1])[0, 0], x), allow_input_downcast=True)

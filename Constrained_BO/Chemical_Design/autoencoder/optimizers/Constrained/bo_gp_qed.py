...
generating molecules that are optimised for QED, SA and don't incur a 
ring-penalty.
"""
...
X = np.loadtxt('../qed_features_and_targets/latent_faetures.txt')
y = -np.loadtxt('../qed_features_and_targets/targets.txt')
y = y.reshape((-1, 1)) 
...
for iteration in range(num_iters):
    ...
    # save_object(bb_alpha, "results_QED_comp/bb_alpha{}.dat".format(iteration))
    ...
    save_object(num_C, "results_QED_comp/num_C_samples{}.dat".format(iteration))
    save_object(num_val, "results_QED_comp/num_val_samples{}.dat".format(iteration))
    
    save_object(num_sensibles, "results_QED_comp/num_sensible{}.dat".format(iteration))
    save_object(num_sensible_and_longs, "results_QED_comp/num_sensible_and_long{}.dat".format(iteration))
    ...
    save_object(valid_smiles_final, "results_QED_comp/valid_smiles{}.dat".format(iteration))
    save_object(all_smiles_final, "results_QED_comp/all_smiles{}.dat".format(iteration))
    save_object(con_scores, "results_QED_comp/con_scores{}.dat".format(iteration))
    save_object(probs, "results_QED_comp/probs{}.dat".format(iteration))
    save_object(log_probs, "results_QED_comp/log_probs{}.dat".format(iteration))

    qed_values = np.loadtxt('../qed_features_and_targets/qed_values.txt')
    SA_scores = np.loadtxt('../qed_features_and_targets/SA_scores.txt')
    ...
    targets = qed_values_normalized + SA_scores_normalized + cycle_scores_normalized
    ...
    for i in range(len(valid_smiles_final)):
        ...
        if len(valid_smiles_final[i]) != 0:
            for j in range(0, len(valid_smiles_final[i])):
                ...
                score = (current_qed_value_normalized + current_cycle_score_normalized + current_SA_score_normalized)
                to_add.append(-score)
    ...
    save_object(reg_scores, "results_QED_comp/reg_scores{}.dat".format(iteration))
    save_object(qed_scores, "results_QED_comp/qed_scores{}.dat".format(iteration))
    save_object(SA_values, "results_QED_comp/SA_scores{}.dat".format(iteration))

"""
This module contains the Sparse_GP class.
"""
...
class Sparse_GP:
    ...
    def __init__(self, n_inducing_points, n_points, input_d, input_means, input_vars, training_targets):  # input means and vars come from bo_gp.py
        ...
        # These are the actual parameters of the posterior distribution being optimized
    ...
    def compute_log_averaged_ei(self, x, X, incumbent):
        ...
        # We compute the predictions once we add the new data points at X with mean given by the GP predictive mean
        ...
        meanCavity = T.concatenate([meanCavity, m_old_X], 0)
        covCavity = T.concatenate([covCavity, T.zeros(KXz.shape)], 0)
        toAdd = T.concatenate([T.zeros(KXz.shape).T, T.eye(KXz.shape[0]) * self.jitter * T.exp(self.lsf)], 0)
        covCavity = T.concatenate([covCavity, toAdd], 1)

        Kzz_expandedInvcovCavity = T.dot(Kzz_expandedInv, covCavity)
        Kzz_expandedInvmeanCavity = T.dot(Kzz_expandedInv, meanCavity)
        Kxz_expanded = compute_kernel(self.lls, self.lsf, x, z_expanded)
        B = T.dot(Kzz_expandedInvcovCavity, Kzz_expandedInv) - Kzz_expandedInv
        v_out = T.exp(self.lsf) + T.dot(Kxz_expanded * T.dot(Kxz_expanded, B), T.ones_like(z_expanded[:, 0: 1]))  # + T.exp(self.lvar_noise)
        m_out = T.dot(Kxz_expanded, Kzz_expandedInvmeanCavity)
        s = (incumbent - m_out) / T.sqrt(v_out)
